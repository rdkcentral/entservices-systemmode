...
*/
    ...
    inline bool getSystemModePropertyValue(const std::string& systemMode, const std::string& property, std::string& value)
    {
        if (systemMode.empty() || property.empty() ) {
            LOGINFO("Error: systemMode or property is empty. systemMode: %s property: %s ",systemMode.c_str(),property.c_str());
            return false;
    ...
    inline std::string replaceString(std::string sentence, const std::string& oldString, const std::string& newString) {
